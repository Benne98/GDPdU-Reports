r"""Bulk-load personnel subledger (personaltable.xlsx per year) into fact_personnel_employee.

USAGE (PowerShell)::

    $env:DB_PASSWORD='<password>'; $env:DB_NAME='finssentials_v2'
    backend\.venv\Scripts\python.exe backend\scripts\load_personnel_subledger.py `
...
from app.services.personnel_ingest import (  # noqa: E402
    _INSERT_COLUMNS,
    as_of_date_for_year,
    map_personnel_rows,
)
...
def _load_one(year: int, root: Path, dry_run: bool, project_id: str) -> int:
    path = root / str(year) / "personaltable.xlsx"
    if not path.exists():
        raise SystemExit(f"missing source file: {path}")
    df = pd.read_excel(path)
    df.columns = [str(col).strip() for col in df.columns]
    rows = map_personnel_rows(df, year=year, source_file_id=path.name, project_id=project_id)
    ...
    insert_sql = text(
        f"INSERT INTO fact_personnel_employee ({', '.join(_INSERT_COLUMNS)}) VALUES ({placeholders})"
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Load personnel subledger into Postgres.")
