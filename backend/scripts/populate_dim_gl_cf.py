r"""Populate ``dim_gl_cf`` from the CF mapping library (``cf_mapping_library``).
...
account's classification to the library:
  ...
  BS / Net-asset accounts:
      dim_gl_na (l6_na_mapping, l7_na_description)
        ⋈ cf_mapping_library (key_kind='na', key_1=l6_na_mapping,
                              key_2=l7_na_description)

  P&L accounts:
      dim_gl_account (level_0='PL', level_3)
        ⋈ cf_mapping_library (key_kind='pl_level3', key_1='PL', key_2=level_3)
...
FROM dim_gl_na na
LEFT JOIN cf_mapping_library lib
...
def _report_unmatched(session: SASession) -> tuple[int, int]:
    ...
    if na_un:
        print(f"[unmatched NA] {len(na_un)} (NA, NA Description) classification(s) with NO library row "
              f"— extend cf_mapping_library to cover these:")
        for r in na_un:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Populate dim_gl_cf from cf_mapping_library.")
    parser.add_argument("--dry-run", action="store_true",
                        help="report match/unmatched counts only; write nothing")
    args = parser.parse_args()

    with SASession(engine) as session:
        lib_n = session.execute(text("SELECT COUNT(*) FROM cf_mapping_library")).scalar() or 0
        if lib_n == 0:
            print("ERROR: cf_mapping_library is empty — run load_cf_mapping_library.py first.")
            return 2
        before = session.execute(text("SELECT COUNT(*) FROM dim_gl_cf")).scalar() or 0
        print(f"cf_mapping_library rows: {lib_n}   dim_gl_cf rows before: {before}")
