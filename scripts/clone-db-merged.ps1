# Clone finssentials_v2 -> finssentials_merged for the unified "merged" stack (port 5180 / API 8014).
# Source is READ-only (pg_dump). finssentials_v2 is the richest reporting substrate
# (plan-seed, mart, subledgers, fixed-asset/personnel). Budget rows are created LIVE via the
# merged UI afterwards. Target name is lowercase -> passes the live-DB guard (keyed on literal "Finssentials").
#
#   $env:DB_PASSWORD = "<postgres-password>"; .\scripts\clone-db-merged.ps1
...
$dumpFile = Join-Path $env:TEMP "finssentials_merged_clone.dump"
