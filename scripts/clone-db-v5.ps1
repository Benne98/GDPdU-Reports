# Clone finssentials_v2 -> finssentials_v5 for the reworked "v5" pipeline stack (port 5181 / API 8015).
# Source is READ-only (pg_dump). finssentials_v2 is the richest reporting substrate, matching the
# merged clone. v5 is the fundamentally-reworked, dataset-agnostic Project-Setup -> Reporting pipeline;
# the 5180 merged stack stays UNTOUCHED as the numeric parity baseline. Target name is lowercase ->
# passes the live-DB guard (keyed on literal "Finssentials").
#
#   $env:DB_PASSWORD = "<postgres-password>"; .\scripts\clone-db-v5.ps1
...
$dumpFile = Join-Path $env:TEMP "finssentials_v5_clone.dump"
