...
Other taxes       = its OWN leaf (cf_mapping='Other taxes'), placed directly AFTER
                    Taxes on income → part of Gross cash flow — see below
```

**`Other taxes` is its own leaf inside the Gross cash flow build-up.** It is a real
non-cash P&L expense (KFZ-Steuer, Grundsteuer, Grundbesitzabgaben) that WAS previously
left unmapped and so dropped out of the indirect Cash Flow — making the Net cash flow
NOT tie to the actual change in cash & cash equivalents (FY2023: Net CF `8,837` €k vs
...
(`dim_cf_structure` row `CF_OTHER_TAXES`, kpi_code `CF:detail`) places **DIRECTLY AFTER
`Taxes on income`** and immediately **BEFORE the `Gross cash flow` subtotal** (details=0
→ top-level, not nested under a cluster subtotal). Because the subtotal logic computes
`Gross cash flow = Σ pre-WC leaves`, this placement folds Other taxes INTO Gross cash
flow (a product/presentation decision). Resulting identities:

- `CF EBITDA == P&L EBITDA`  — **still LOCKED** (Other taxes ∉ the EBITDA leaf set; it
  is a separate `cf_mapping` leaf, never summed into EBITDA).
- `Gross cash flow == EBITDA + Taxes on income + Other taxes`  — Other taxes is now
  **inside** Gross cash flow (the previous `EBITDA + Taxes` identity is intentionally
  superseded by this presentation decision).
- `Net cash flow == ΔCash`  — **unchanged**; only the leaf's POSITION moved, the Net
  total still Σ's ALL presented non-cash leaves (== −Σ of the cash & cash-equivalents
  leaves).

Sign convention unchanged (single `× −1` in SQL). The `_PL_LEVEL3_TO_CF` key
`'Other taxes'` catches `KFZ-Steuer` (level_3='Other taxes') via the level_3 pass and
`Grundsteuer`/`Grundbesitzabgaben` (level_2='Other taxes', level_3='Other') via the
level_2 fallback (`etl.cf_fill`); `level_3='Other'` under OTHER level_2 categories does
...
`Taxes on income`. Other taxes −5 → its OWN `Other taxes` leaf placed directly AFTER
Taxes on income. `Gross cash flow` = EBITDA + Taxes + Other taxes = 300 − 100 − 5 =
**+195** (Other taxes now inside Gross cash flow). CF `EBITDA` is still **+300** (Other
taxes ∉ EBITDA leaf), and `Net cash flow` = Σ all leaves still includes the −5, so Net
ties to ΔCash.
...
EBITDA + Taxes on income + Other taxes` (Other taxes folded in when present, else
`== EBITDA + Taxes`); Net cash flow non-zero == Σ all mapped leaves; year-grain
EBITDA non-zero. `TestCfOtherTaxesLeaf` (DB-free) locks the `Other taxes` leaf: with
it placed directly after Taxes on income, `Gross cash flow == EBITDA + Taxes on
income + Other taxes` and `Net cash flow == Σ all leaves` (reconciles to ΔCash) while
`CF EBITDA == P&L EBITDA` stays UNCHANGED (Other taxes ∉ EBITDA leaf).
