...
| 5 | Auto-extension Project-Setup step (dec. 2) | ✅ **DONE + LIVE-VERIFIED** (2026-07-07). `structure_autoextend.py` (detect via readers' `_match_grain` + reader-faithful placement writer); `POST /ingest/structure/unknown-positions` (read) + `/ingest/structure/extend` (admin write); migration `0032` (additive `source` col) applied to v5; new wizard step 7 "Statement structure" (auto-passes when no unknowns). Live v5: 2832 CoA accts, 0 unknowns → step no-op; extend tested in rolled-back txn (insert +1 PL, idempotent skip on repeat, BS/CF untouched, rollback clean). 43 tests; formula/rules in financial-logic.md. Structure tables global (tenant caveat) |
...
| 7 | Remove entity dropdown; multi-select filter; conditional display (dec. 6/7) | ✅ **DONE + LIVE-VERIFIED** (2026-07-07). IS/BS/CF/WC always consolidated (dropdown removed, numbers unchanged = old entity='all' path); multi-select via comma-sep on existing `entity` param (`resolve_entity_prefixes`: 'all'/None→[], '01'→['01'], '01,02'→['01','02']) on Profitability/Payroll/FA/OPOS; conditional display via `GET /meta/reporting-availability` (payroll/fixed_assets/opos each try/except→False). Phase-4 UI follow-on done: `PlanVersionPanel` (version select + include toggle, hides when supported=false). 16+ tests; build OK. Follow-on: restricted-user multi-entity narrowing on aging (over-broad within visibility, no cross-tenant leak) |
| 8 | Real DSO/DPO (dec. 8) | ✅ **DONE + LIVE-VERIFIED** (2026-07-07). DSO from `fact_sales.gross_sales`; DPO from GL-derived COST_OF_MATERIALS (runtime-resolved from dim_pl_structure, not hardcoded). Numerator = **gross** open aging basis (reconciles with displayed AR/AP — a net-basis defect DSO=4.4d was caught in live-verify + fixed to 32.6d). Proxy fallback (AR30/AP45) retained via `dso_source`/`dpo_source` flag; golden-safe. Live v5: DSO 32.6d / DPO 46.1d, both `source='real'`. 54 tests. No purchases journal in v5 → DPO is GL-derived (documented). Router 501 `derive_aging` stub kept (live, test-pinned — not the dead one) |
| 9 | FA annual-only pills; keep pass-through (dec. 9) | ✅ **DONE** (2026-07-07, commit `b84fade`). `annualOnly` mode on ModulePeriodFilterBar (Monthly/Weekly disabled+greyed, Annual forced) gated on the fixed-assets sub-tab via `annualOnlySubTabs`. Depreciation stays pass-through; F1/F2 deferred. Frontend-only |
