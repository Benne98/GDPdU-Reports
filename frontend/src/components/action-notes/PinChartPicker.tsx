...
import type { ChartPinCandidate } from './ActionNotesContext'

type Props = {
  candidates: ChartPinCandidate[]
  onClose: () => void
  onConfirm: (candidateId: string, restoreMode: 'navigate' | 'preview_only') => void
}

export default function PinChartPicker({ candidates, onClose, onConfirm }: Props) {
  ...
  if (!candidates.length) {
    return (
      <div className="fixed inset-0 z-[62] flex items-center justify-center p-4 bg-slate-900/40" onClick={onClose}>
        ...
        >
          <p className="text-sm text-slate-700">No pinnable charts on this page. Open a statement with chart data first.</p>
  ...
  return (
    <div className="fixed inset-0 z-[62] flex items-center justify-center p-4 bg-slate-900/40" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="pin-chart-picker-title"
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <h3 id="pin-chart-picker-title" className="text-sm font-semibold text-slate-900 flex items-center gap-2">
            <Image size={16} className="text-amber-600" />
            Pin a chart screenshot
          </h3>
        ...
        <div className="p-4 space-y-3 max-h-[50vh] overflow-y-auto">
          <p className="text-[0.65rem] text-slate-500">
            Choose which chart to capture. A screenshot will be taken and saved to your pins.
          </p>
          ...
          <div className="pt-2 space-y-2">
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              ...
              />
              <span className="text-xs text-slate-700">
                <strong>Preview only</strong> — screenshot saved in Action Notes, no navigation
              </span>
            </label>
          </div>
        </div>
