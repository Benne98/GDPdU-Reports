...
import { exportSupplierRegister } from '../shared/supplierRegisterExport'
...
function DocumentLinesTable({ docs }: { docs: PayablesSupplierRegisterDocument[] }) {
  return (
...
function DocumentDetailPanel({
  ...
  row: PayablesSupplierRegisterRow
  year: number
  month: number
  entity?: string
}) {
  const [docs, setDocs] = useState<PayablesSupplierRegisterDocument[]>(row.documents ?? [])
  ...
  useEffect(() => {
    ...
    setError(null)
    api
      .salesPayablesSupplierDocuments(row.supplier_id, year, month, entity)
      .then(res => {
  ...
  }, [row.supplier_id, row.documents, row.open_documents, year, month, entity])
...
export default function PayablesSupplierRegister({
  rows,
  filterSupplier,
  onClearFilter,
  year,
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => r.supplier_name === filterSupplier)
  }, [rows, filterSupplier])
  ...
  const columns = useMemo((): FilterableColumn<PayablesSupplierRegisterRow>[] => [
    ...
    },
    {
      ...
      align: 'right',
      getValue: r => String(r.cost_of_materials ?? r.procurement_spend ?? 0),
    ...
    <div>
      ...
      <p className="text-xs mt-0.5" style={{ color: BRAND.textMuted }}>
        {filterSupplier
          ? (
            <>
              Filtered to <strong style={{ color: BRAND.navy }}>{filterSupplier}</strong>
              {onClearFilter ? (
                <>
                  {' · '}
                  <button
          ...
          : 'All suppliers — expand a row for open documents · select a bubble above to focus'}
      </p>
    </div>
  )
