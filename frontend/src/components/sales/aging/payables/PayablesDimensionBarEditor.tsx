...
import {
  PAYABLES_DIMENSION_BAR_OPTIONS,
  type PayablesDimensionBarConfig,
} from '../shared/payablesDimensionBarConfig'

export default function PayablesDimensionBarEditor({
  config,
  onChange,
  disabled,
}: {
  config: PayablesDimensionBarConfig
  onChange: (cfg: PayablesDimensionBarConfig) => void
  disabled?: boolean
}) {
    ...
    <div className="shrink-0">
      ...
      <SalesChartEditorPopover open={open} onClose={() => setOpen(false)} anchorRef={anchorRef} width={280}>
        ...
        <div className="flex flex-col gap-0.5 mb-3 shrink-0 max-h-40 overflow-y-auto">
          {PAYABLES_DIMENSION_BAR_OPTIONS.map(opt => {
