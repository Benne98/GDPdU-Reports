import type { ReceivablesConcentrationTrendPoint } from '../../../../lib/api'
...
} from './agingConcentrationBands'
import {
  buildConcentrationTrendSubtitle,
  type AgingConcentrationTrendConfig,
} from './agingConcentrationTrendConfig'
import { normalizeReceivablesConcentration } from './normalizeConcentration'
...
function trendRows(
  ...
  const rows: XlsxRow[] = points.map(p => {
    const normalized = normalizeReceivablesConcentration({
      year: p.year,
      month: p.month,
...
export async function exportReceivablesConcentrationTrend(opts: {
  kind: PlExportKind
  points: ReceivablesConcentrationTrendPoint[]
  ...
  const base = `Receivables_Concentration_Trend_${todayStr()}`
  const subtitle = buildConcentrationTrendSubtitle(config, periodGrain)

  if (kind === 'pptx') {
    await exportFlatTablePptx({
      fileName: `${base}.pptx`,
      pageTitle: 'Customer concentration trend',
  ...
  await exportToXlsx({
    title: 'Customer concentration trend',
    subtitle,
    headers,
    rows,
    filename: `${base}.xlsx`,
