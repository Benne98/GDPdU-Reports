...
import { api, type ReceivablesConcentration, type ReceivablesConcentrationTrendPoint } from '../../../lib/api'
...
import ReceivablesConcentrationCard from './ReceivablesConcentrationCard'
import ReceivablesConcentrationTrend from './ReceivablesConcentrationTrend'
...
import { exportReceivablesConcentrationTrend } from './shared/receivablesConcentrationTrendExport'
import { normalizeReceivablesConcentration } from './shared/normalizeConcentration'
...
export default function ReceivablesConcentrationSection({
  period,
  year,
  month,
  entity,
  ...
  const [concentration, setConcentration] = useState<ReceivablesConcentration | null>(null)
  ...
  const [trendPoints, setTrendPoints] = useState<ReceivablesConcentrationTrendPoint[]>([])
  ...
  useEffect(() => {
    ...
    api
      .salesReceivablesConcentration(year, month, entity, {
      ...
      .then(raw => {
        if (cancelled) return
        setConcentration(normalizeReceivablesConcentration(raw))
      })
      .catch(err => {
        if (cancelled) return
  ...
  const loadTrend = useCallback(async () => {
    ...
    try {
      const r = await api.salesReceivablesConcentrationTrend(year, month, entity, 12, {
        period_grain: periodGrain,
        iso_year: periodOpts.iso_year as number | undefined,
        iso_week: periodOpts.iso_week as number | undefined,
  ...
  async function handleTrendExport(kind: PlExportKind) {
    ...
    await exportReceivablesConcentrationTrend({
      kind,
      points: trendPoints,
      config: trendConfig,
      periodGrain,
  ...
  return (
    <OperationalCard
      ...
      subtitle="Open receivables by non-overlapping customer rank"
    ...
    >
      ...
      >
        {concentration && <ReceivablesConcentrationCard data={concentration} />}
      </AgingChartFrame>
