...
import {
  AGING_TREND_GRAIN_OPTIONS,
  AGING_TREND_PERIOD_OPTIONS,
  RECEIVABLES_TREND_METRICS,
  type AgingTrendChartConfig,
  type ReceivablesTrendMetricId,
} from './shared/agingTrendChartConfig'

export default function ReceivablesTrendChartEditor({
  config,
  onChange,
  disabled,
}: {
  config: AgingTrendChartConfig
  onChange: (cfg: AgingTrendChartConfig) => void
  disabled?: boolean
}) {
  const [open, setOpen] = useState(false)
  const anchorRef = useChartEditorAnchor()

  function toggleMetric(id: ReceivablesTrendMetricId) {
    ...
    <div className="shrink-0">
      ...
      <SalesChartEditorPopover open={open} onClose={() => setOpen(false)} anchorRef={anchorRef} width={300}>
        ...
        <div className="flex flex-wrap gap-1 mb-3 shrink-0">
          {AGING_TREND_GRAIN_OPTIONS.map(opt => {
        ...
        <div className="flex flex-wrap gap-1 mb-3 shrink-0">
          {AGING_TREND_PERIOD_OPTIONS.map(opt => {
        ...
        <div className="flex flex-col gap-1 max-h-48 overflow-y-auto shrink-0">
          {RECEIVABLES_TREND_METRICS.map(m => (
            <label
              key={m.id}
