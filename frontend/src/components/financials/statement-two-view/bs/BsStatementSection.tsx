...
import PlColumnEditor from '../../pl-two-view/PlColumnEditor'
...
import { buildClientBsNarrative } from './bsNarrativeEngine'
...
export default function BsStatementSection({
  data,
  monthly,
  loading,
  error,
  ...
  const cfg = getStatementConfig('bs')
  const groupLabel =
    entityDisplayName ?? (entityLabel === 'all' ? 'All entities (consolidated)' : entityLabel)
  const [viewMode, setViewMode] = useState<StatementViewMode>(() => loadStatementViewMode('bs'))
  const [detailBullet, setDetailBullet] = useState<BsNarrativeBullet | null>(null)
  ...
  const { checkOpen } = usePlRowExpansion(data?.rows, data?.statement ?? 'bs')
  ...
  useEffect(() => {
    saveStatementViewMode('bs', viewMode)
  }, [viewMode])

  useEffect(() => {
    if (!notesCtx || !data) {
      if (notesCtx) notesCtx.unregisterTableCandidate('bs-statement')
      return
    }
    const cols = viewMode === 'table' ? displayColumns : miniColumns
    const component = viewMode === 'table' ? 'BsTableView' : 'BsReportView'
    notesCtx.registerTableCandidate({
      id: 'bs-statement',
      label: cfg.pinTableLabel,
      capture: () => buildPlTablePinSnapshot(data, checkOpen, cols, component),
      viewState: { view_mode: viewMode, tab: 'bs' },
    })
    return () => notesCtx.unregisterTableCandidate('bs-statement')
  }, [notesCtx, data, viewMode, displayColumns, miniColumns, checkOpen, cfg.pinTableLabel])

  const clientNarrative = useMemo(
    () => (data ? buildClientBsNarrative(data, year, month, 5) : null),
    [data, year, month],
  )

  const exportBulletsRaw = useStatementExportBullets(data, narrative, clientNarrative)

  const exportBullets = useMemo(() => {
    if (!data?.rows.length) return exportBulletsRaw
    const intro = narrative?.intro ?? clientNarrative?.intro
    const visibleTableRows = countVisibleStatementRows(data.rows, checkOpen)
    return fitBulletsToTable(exportBulletsRaw, visibleTableRows, intro, 5)
  }, [data?.rows, exportBulletsRaw, checkOpen, narrative?.intro, clientNarrative?.intro])
      ...
      >
        <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {viewMode === 'table' && cfg.features.columnEditor && data && (
              <PlColumnEditor
