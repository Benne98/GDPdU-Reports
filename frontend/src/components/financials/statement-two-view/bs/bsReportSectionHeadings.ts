...
export function buildBsReportTableHeading(
  ...
  if (opts?.entityDisplayName?.trim()) {
    return `${opts.entityDisplayName.trim()}'s Balance sheet — ${cm} vs ${pm}`
  }
  return `Consolidated Balance sheet — ${cm} compared to ${pm}`
}
