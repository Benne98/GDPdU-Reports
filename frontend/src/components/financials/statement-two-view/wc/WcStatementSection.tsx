...
import { buildClientWcNarrative } from './wcNarrativeEngine'
import {
...
export default function WcStatementSection({
  data,
  monthly,
  loading,
  error,
  ...
  const cfg = getStatementConfig('wc')
  const groupLabel =
  ...
  const { checkOpen } = usePlRowExpansion(data?.rows, data?.statement ?? 'wc')
  ...
  useEffect(() => {
    saveStatementViewMode('wc', viewMode)
  }, [viewMode])

  useEffect(() => {
    if (!notesCtx || !data) {
      if (notesCtx) notesCtx.unregisterTableCandidate('wc-statement')
      return
    }
    const cols = viewMode === 'table' ? displayColumns : miniColumns
    const component = viewMode === 'table' ? 'WcTableView' : 'WcReportView'
    notesCtx.registerTableCandidate({
      id: 'wc-statement',
      label: cfg.pinTableLabel,
      capture: () => buildPlTablePinSnapshot(data, checkOpen, cols, component),
      viewState: { view_mode: viewMode, tab: 'wc' },
    })
    return () => notesCtx.unregisterTableCandidate('wc-statement')
  }, [notesCtx, data, viewMode, displayColumns, miniColumns, checkOpen, cfg.pinTableLabel])

  const clientNarrative = useMemo(
    () => (data ? buildClientWcNarrative(data, year, month, 5) : null),
    [data, year, month],
  )
